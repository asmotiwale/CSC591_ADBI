...
def computeDelta(wt, X, Xi):
    ...
    """
    # YOUR CODE GOES HERE
    def computeSimilarity(X, Xi, wt):
    	s = 0
    	mean_X = np.mean(X[:-1])
    	mean_Xi = np.mean(Xi[:-1])
        std_X = np.std(X[:-1])
        std_Xi = np.std(Xi[:-1])
    	for i in range(0, len(X) - 1):
    		s += (X[i] - mean_X) * (Xi[i] - mean_Xi)

        similarity = np.exp(wt * s / ((len(X) - 1) * std_X * std_Xi))     
    	return similarity

    s, t = 0, 0    
    for i in range(0, len(Xi)):
        s += Xi.iloc[i][-1] * computeSimilarity(X, Xi.iloc[i], wt)

    for i in range(0, len(Xi)):
        t += computeSimilarity(X, Xi.iloc[i], wt)
...
linear_model = smf.ols(formula = 'deltaP ~ deltaP90 + deltaP180 +deltaP360', data = trainData)
