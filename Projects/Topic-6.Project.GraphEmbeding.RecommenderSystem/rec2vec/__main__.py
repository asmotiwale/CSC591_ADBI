...
def process(args):
    ...
    # YOUR CODE HERE
    # Performing the random walk using Deep Walk.
    w = graph.build_deepwalk_corpus(G, args.number_walks, args.walk_length) 

    # Performing embedding using word2Vec.
    model = Word2Vec( w, size = args.representation_size, window = args.window_size, min_count = 0,workers = args.workers)
...
def predict_rating(model, nodedict, user, movie):
    ...
    res = 0
    for i in range(1, 6):
        m_sim = model.wv.similarity(nodedict[user].id, nodedict[movie+"_"+str(i)].id)
        if m_sim > s:
            s = m_sim
            res = i
    return res        
