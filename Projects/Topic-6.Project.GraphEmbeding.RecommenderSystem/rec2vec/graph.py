...
def records_to_graph():
    ...
    ii = 0
    #print movies['m55577'].name
    for m,i in movies.iteritems():
      nn = Node(ii, m, 'movie')
      nodelist.append(nn)
      nodedict[m] = nn
      ii += 1
    ...
    # Creating nodes for director      
    for d in directors:
      nn = Node(ii, d, "director")
      nodelist.append(nn)
      nodedict[d] = nn
      ii += 1

    # Creating nodes for genre
    for g in genres:
      # print g
      nn = Node(ii, g, "genre")
      nodelist.append(nn)
    ...
    # Creating nodes for rating
    for m in movies:
      for t in range(1, 6):
        nn = Node(ii, m+"_"+str(t), "rating")
        nodelist.append(nn)
        nodedict[m+"_"+str(t)] = nn
        ii += 1
    ...
    for u, d in ratings.iteritems():
      for m, r in d.iteritems():
        nodedict[u].neighbors.append(nodedict[m+"_"+r])

    # Adding edges between movies and directors
    for m in movies:
      if movies[m].director:
        nodedict[m].neighbors.append(nodedict[movies[m].director])
    # print nodedict["m1"].neighbors[0].name

    # Adding edges between movies and actors
    for m in movies:
      if movies[m].actors:
        for a in movies[m].actors:
          nodedict[m].neighbors.append(nodedict[a])
    # print nodedict["m1"].neighbors[-1].name, nodedict["m1"].neighbors[-1].type

    # Adding edges between movies and genres
    for m in movies:
      if movies[m].genres:
        for g in movies[m].genres:
          nodedict[m].neighbors.append(nodedict[g])
    #print nodedict["m1"].neighbors[-1].name, nodedict["m1"].neighbors[-2].name, nodedict["m1"].neighbors[-3].name, nodedict["m1"].neighbors[-3].type  

    # Adding edges between movie ratings and movies
    for m in movies:
      for t in range(1, 6):
        nodedict[m].neighbors.append(nodedict[m+"_"+str(t)])   
